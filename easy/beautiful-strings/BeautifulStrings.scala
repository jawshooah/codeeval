...
object Main extends App {
  ...
  for (line <- lines) {
    println(maximumBeauty(line))
  }

  final val MaxBeauty = 26

  def maximumBeauty(line: String): Int = {
    val letters = line.filter{ _.isLetter }.map{ _.toUpper }
    val counts = letters.distinct.map{ c => letters.count{ _ == c } }.sorted
    val beautyRange = (MaxBeauty - counts.length + 1) to MaxBeauty
    counts.zip(beautyRange).map{ case (c, b) => c * b }.sum
  }
}
