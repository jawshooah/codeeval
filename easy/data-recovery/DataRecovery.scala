...
object Main extends App {
  ...
  final val WordSep = " "
  final val SentenceSep = ";"

  for (line <- lines) {
    val Array(wordStr, indexStr) = line split SentenceSep
    val words = wordStr split WordSep
    val indices = indexStr split WordSep map { _.toInt }

    val wordArray = Array.fill(words.length)("")

    for ((index, word) <- indices zip words) {
      wordArray(index - 1) = word
    }

    val missingIndex = wordArray indexOf ""

    if (missingIndex > -1) {
      wordArray(missingIndex) = words.last
    }
